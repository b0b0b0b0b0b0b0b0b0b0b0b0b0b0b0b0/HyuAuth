...
public class PlayerBlockBreakSystem extends com.hypixel.hytale.component.system.EntityEventSystem<EntityStore, PlaceBlockEvent> {
    ...
    public void handle(int index, ArchetypeChunk<EntityStore> archetypeChunk, Store<EntityStore> store, CommandBuffer<EntityStore> commandBuffer, PlaceBlockEvent event) {
        System.out.println("[HyuAuth] [EVENT-LOG] PlayerBlockBreakSystem.handle() CALLED | index=" + index + " | event=" + event.getClass().getName());
        try {
            UUIDComponent component = archetypeChunk.getComponent(index, UUIDComponent.getComponentType());
            System.out.println("[HyuAuth] [EVENT-LOG] PlayerBlockBreakSystem | UUIDComponent=" + (component != null ? "found" : "null"));
            if (component != null) {
                @SuppressWarnings("deprecation")
                UUID playerUuid = component.getUuid();
                System.out.println("[HyuAuth] [EVENT-LOG] PlaceBlockEvent (BREAK) | Player: " + playerUuid + " | Action: BLOCK_BREAK");
                boolean isLoggedIn = authManager.isLoggedIn(playerUuid);
                if (!isLoggedIn) {
                    System.out.println("[HyuAuth] [EVENT-LOG] PlaceBlockEvent (BREAK) | BLOCKED for " + playerUuid);
                    event.setCancelled(true);
                }
            } else {
                System.out.println("[HyuAuth] [EVENT-LOG] PlaceBlockEvent (BREAK) | Player: UNKNOWN | No UUIDComponent at index " + index);
            }
        } catch (Exception e) {
            System.out.println("[HyuAuth] [EVENT-LOG] PlaceBlockEvent (BREAK) | ERROR: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
